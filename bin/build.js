...
if (!package.type) {
  ...
  console.log(
    '\x1b[41mShould be one of : "section", "wwobject", "plugin".\x1b[0m'
  );
  return;
}
if (types.indexOf(package.type.toLowerCase()) === -1) {
  console.log(
    '\x1b[41mError : "type" in package.json should be one of : "section", "wwobject", "plugin".\x1b[0m'
  );
  return;
}
...
if (!fs.existsSync("./ww-config.json")) {
  ...
  console.log(
    '\x1b[44mTo create "./ww-config.json" use "yarn create-config".\x1b[0m'
  );
  return;
}
...
if (!config.componentPath) {
  ...
  console.log(
    '\x1b[44mProperty "componentPath" not found in "ww-config.json". Using "' +
      componentPath +
      '".\x1b[0m'
  );
} else {
...
if (!fs.existsSync(componentPath)) {
  console.log(
    '\x1b[41m"' +
      componentPath +
      '" not found. Please check "componentPath" in "./ww-config.json".\x1b[0m'
  );
}
