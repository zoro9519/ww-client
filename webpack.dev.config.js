...
if (packageJson.type.toLowerCase() == "section") {
  ...
  try {
    ...
    console.log(
      "\x1b[41mError : " +
        basePath +
        "/index.js incorrect format, or no data defined\x1b[0m"
    );
    return;
  }
  ...
  for (let sectionType of packageJson.sectionTypes) {
    if (!sectionType.name) {
      console.log(
        "\x1b[33mWarning : name not set for ",
        sectionType + "\x1b[0m"
      );
      continue;
    }
    ...
    try {
      ...
      console.log(
        "\x1b[33mWarning : " +
          ...
          sectionType.name +
          "/data.json" +
          " not found or incorrect format\x1b[0m"
      );
      continue;
    }
    ...
    for (let i = 0; i < 10; i++) {
      if (
        fs.existsSync(
          basePath + "/" + sectionType.name + "/preview_" + i + ".jpg"
        )
      ) {
      ...
      if (
        fs.existsSync(
          basePath + "/" + sectionType.name + "/preview_" + i + ".png"
        )
      ) {
        sectionType.previews.push({
...
fs.writeFileSync("./assets/info.json", JSON.stringify(packageJson), function(
  err
) {
  if (err) {
    throw new Error();
